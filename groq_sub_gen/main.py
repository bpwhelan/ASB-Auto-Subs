...
GROQ_API_KEY = "" # Replace with your actual key if not using environment variables
...
# --- Main Clipboard Monitoring Logic ---
    ...
    while True:
        try:
            ...
            if current_clipboard_content != previous_clipboard_content and current_clipboard_content:
                previous_clipboard_content = current_clipboard_content # Update immediately
                if is_youtube_url(current_clipboard_content):
                    logging.info(f"Detected YouTube link: {current_clipboard_content}")
                    audio_file_path = None # Ensure defined for cleanup
                    try:
                        # 1. Download Audio
                        ...
                        if audio_file_path and os.path.exists(audio_file_path):
                            ...
                            # 2. Generate Subtitles using SubtitleProcessor
                            ...
                            try:
                                srt_path, _ = processor.generate_subtitles(
                                    ...
                                    timestamp_granularities_str="word", # "word" or "segment"
                                    # auto_detect_language=True, # Or set language='en', auto_detect_language=False
                                    language='ja',
                                    auto_detect_language=False,
                                    # ----------------------------------------
                                    include_video=False # Not embedding video here
                                )
                                if srt_path:
                        ...
                        # 3. Clean up downloaded audio file regardless of success/failure
                        if audio_file_path and os.path.exists(audio_file_path):
                            try:
                ...
                # else: # Optional: Log if clipboard changed but wasn't a YT link
                #    logging.debug("Clipboard changed, but not a YouTube link.")

            time.sleep(1) # Check clipboard every second

        except pyperclip.PyperclipException as clip_err:
             # Handle potential clipboard access issues, especially on some systems/servers
             logging.warning(f"Could not access clipboard: {clip_err}. Retrying...")
             time.sleep(5) # Wait longer if clipboard access fails
...
if __name__ == "__main__":
    ...
    # Check for ffmpeg (optional but good practice)
    try:
        ...
        # Decide if you want to exit or just warn. yt-dlp might still work for some formats.
        # exit(1)
